                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight" style="position:relative">
                            <i class="fa-regular fa-user"></i> <i>{{ $unit->name }}</i> Unit
                        </h3>
                    </div>
                    <div class="card-body">
                        <p>{{ __('name') }}: {{ $unit->name }}</p>
                        <p>{{ __('comment') }}: {{ $unit->comment }}</p>

                        <p>{{ __('Created at') }}: {{ $unit->created_at }}</p>
                        <p>{{ __('Updated at') }}: {{ $unit->updated_at }}    by {{ $unit->user->name }}</p>
